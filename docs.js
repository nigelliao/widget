...
$(function() {
	$('body').on('keyup','.e-input',function(e){
		var e = event || window.event;
		
		if(e.keyCode == 13) {
		  var _val = $(this).val();
		  
      if(CheckIsColor(_val) == false){
      	
      	$(this).addClass("active");
      }
      if(CheckIsColor(_val) == true){
      	
      	$(this).removeClass("active");	
			  $(this).parent().find(".full").spectrum("set", _val);
	 //		$(this).parent().find(".full").spectrum("hide");
      }
		}
		
		function CheckIsColor(colorValue) {  
			
//	    正则表达式:"^#([0-9a-fA-F]{6}|[0-9a-fA-F]{3})$"
...
$(".full").spectrum({
    ...
    move: function (color) {
        updateBorders(color);
        $(this).parent().find('.e-input').val(color.toHexString());
    },
    show: function () {
